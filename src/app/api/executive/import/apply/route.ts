      ...
      if (item.action === "create" && item.partner) {
        ...
        if (item.yearlyEvents?.length) {
          for (const ev of item.yearlyEvents) {
            await prisma.executiveYearlyEvent.upsert({
        ...
        const p = item.partner;
        const existing = await prisma.executivePartner.findUnique({
        ...
        if (
          p.phone !== undefined &&
          (p.phone ?? "").trim() !== "" &&
          (p.phone ?? "").trim() !== (existing.phone ?? "").trim()
        ) {
          updates.phone = p.phone?.trim() ?? "";
          historyParts.push("휴대폰 변경");
        }
        if (
          p.companyNormalized != null &&
          (p.companyNormalized ?? "").trim() !== "" &&
          (p.companyNormalized ?? "").trim() !== (existing.companyNormalized ?? "").trim()
        ) {
        ...
        if (Object.keys(updates).length > 0) {
          await prisma.executivePartner.update({
            where: { id: item.partnerId },
