...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const diff = await buildMergeDiff(normalized, prisma);
...
async function buildMergeDiff(
  rows: ParsedRow[],
  prisma: ReturnType<typeof getExecutivePrisma>
): Promise<MergeDiff> {
  ...
  for (const row of rows) {
    ...
    if (name && phone) {
      existing =
        partners.find((p) => p.name.trim() === name && (p.phone ?? "").trim() === phone) ?? null;
    ...
    if (existing) {
      ...
      if (row.phone !== undefined && (row.phone ?? "").trim() !== (p.phone ?? "").trim()) {
        changes.push(`휴대폰 (${empty(p.phone)} → ${empty(row.phone)})`);
      }
      if (
        row.companyNormalized != null &&
        (row.companyNormalized ?? "").trim() !== (p.companyNormalized ?? "").trim()
      ) {
      ...
      if (
        row.businessCardDateRaw !== undefined &&
        (row.businessCardDateRaw ?? "").trim() !== (p.businessCardDateRaw ?? "").trim()
      ) {
