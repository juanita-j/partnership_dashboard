...
const API_ROOT = "/api/executive";

/** 기본 표시 연도. DB에 연도가 생기면 /api/executive/event-years 반영 */
...
function ExecutiveCounterpartContent() {
  ...
  const exportUrl =
    `${API_ROOT}/export/xlsx?${filtersToSearchParams(appliedFilters, eventYears).toString()}&columns=${encodeURIComponent(JSON.stringify(appliedFilters.showColumns))}`;

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-base font-bold">임원진 카운터파트</h1>
      ...
      <ExcelUploadDialog
        ...
        onApplied={refresh}
        apiRoot={API_ROOT}
        importSource="executive_counterpart"
      />
    </div>
  );
}

export default function ExecutiveCounterpartPage() {
  return (
    <Suspense fallback={<div className="p-4">로딩 중...</div>}>
      <ExecutiveCounterpartContent />
    </Suspense>
  );
}
